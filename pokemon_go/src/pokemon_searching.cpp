...
//  waitKey();
    ...
    //      cout << "rect.br().x: " << rect.br().x << endl;
    //     cout << "roi_width-5: " << roi_width-5 << endl;
    ...
    if(MOVE){
        ...
        // cout << "roi_width-5: " << roi_width-5 << endl;
    //   if((rect.tl().x > roi_tl_x)&&(rect.tl().y <roi_tl_y)&&(rect.br().x < roi_br_x)&&(rect.br().y > roi_br_y)){
        if ( (rect.tl().x > 10) && (rect.tl().y > 10) && (rect.br().x < (roi_width-10)) && (rect.br().y < (roi_height-10)) ){
        ROS_INFO("MOVE");
        // Rect rect
